...
// 서버에서 보내주는 플레이어의 id를 이용하여
// 클라이언트를 찾기 위해서 client_map을 사용하는 것
...
thread test_thread; 
// 최대 동접이 될 때까지 클라이언트를 접속을 시키고
// 특정 시간마다 플레이어들이 행동을 수행하도록 작업을 수행하는 쓰레드
...
void Adjust_Number_Of_Client()
{
	...
	ServerAddr.sin_addr.s_addr = inet_addr("211.207.24.227");
...
void Test_Thread()
{
	while (true) {
		...
		// 접속은 운영체제의 오버헤드
		// 한번에 모든 플레이어가 접속을 하도록 하면 
		// 서버가 버틸 수 없음.. 시간차를 두고 접속하는 구조
		// 상용서버에서도 마찬가지임 - 한번에 모든 접속을 할수는없음
